...
// ==UserScript==
// @name         Sideline autodrop + multidrop
...
(function() {
    ...
    // ==================== MULTIDROP QUEUE ====================
    // Array to store totes scanned via MULTIDROP
    let multidropQueue = [];
    ...
    // Function to update the multidrop badge counter
    ...
    // Function to render the tote list inside the multidrop panel
    ...
    // Function to update tote count display
    ...
    async function dropAllMultidrop(dropzone, buttonElement) {
        ...
        for (let i = 0; i < totesCopy.length; i++) {
            ...
            // Small delay between totes to avoid overwhelming the API
        ...
        if (errors.length > 0) {
            const errorsEl = document.getElementById('multidrop-errors');
            if (errorsEl) {
                errorsEl.innerHTML = `<strong>Errores (${errorCount}):</strong><br>${errors.join('<br>')}`;
            }
        }

        // Auto-close after 2 seconds if no errors, 5 seconds if errors
        setTimeout(() => {
    ...
    // *** MODIFIED: R2S checks multidrop mode ***
    readyToStowButton.addEventListener('click', () => {
        if (isMultidropMode && multidropQueue.length > 0) {
            dropAllMultidrop('dz-P-READYTOSTOW', readyToStowButton);
        } else {
    ...
    // *** MODIFIED: RECYCLE checks multidrop mode ***
    recycleButton.addEventListener('click', () => {
        if (isMultidropMode && multidropQueue.length > 0) {
            dropAllMultidrop('dz-P-RECYCLE', recycleButton);
        } else {
            dropTote('dz-P-RECYCLE', recycleButton);
        }
    });

    // Create Edit to Pending button
    ...
    multidropButton.addEventListener('mouseenter', () => {
        ...
        multidropButton.style.width = '160px';
        multidropButton.style.borderRadius = '22.5px';
        multidropButton.style.padding = '0 12px';
        multidropButton.style.justifyContent = 'flex-start';
    ...
    multidropButton.addEventListener('mouseleave', () => {
        ...
        multidropButton.style.width = '45px';
        multidropButton.style.borderRadius = '50%';
        multidropButton.style.padding = '0';
        multidropButton.style.justifyContent = 'center';
    });

    // Create MULTIDROP panel
    ...
    // Create overlay for multidrop panel
    ...
    // MULTIDROP button click - open panel
    multidropButton.addEventListener('click', () => {
        ...
        updateToteCount();
        setTimeout(() => {
            document.getElementById('multidrop-input').focus();
        }, 100);
    });
    ...
    multidropPanel.addEventListener('click', (e) => {
        if (e.target.id === 'multidrop-add-btn') {
            addToteToQueue();
        }
        if (e.target.id === 'multidrop-clear-btn') {
            multidropQueue = [];
            renderToteList();
            updateMultidropBadge();
            updateToteCount();
        }
        if (e.target.id === 'multidrop-close-btn') {
            multidropOverlay.style.display = 'none';
            multidropPanel.style.display = 'none';
        }
    });
    ...
    function addToteToQueue() {
        ...
        if (value && !multidropQueue.includes(value)) {
            ...
            // Flash the input red briefly to indicate duplicate
    ...
    // Create button container for exception buttons
    ...
    // Button configurations for exception buttons
    const otherButtons = [
    ...
    // *** MODIFIED: createButton checks multidrop mode ***
    function createButton(config) {
        ...
        button.addEventListener('click', () => {
            if (isMultidropMode && multidropQueue.length > 0) {
                dropAllMultidrop(config.dropzone, button);
            } else {
...
Revisa la consola (F12) para más información.`);
    ...
    console.log('Sideline Auto-Drop script loaded successfully (v8.0 with MULTIDROP)');
})();
