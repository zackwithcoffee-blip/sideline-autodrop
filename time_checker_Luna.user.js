...
(function() {
    ...
    function getPodBandsURL() {
        const weekStart = getWeekStartEncoded();
        const today = getTodayEncoded();
        return `https://fclm-portal.amazon.com/reports/functionRollup?reportFormat=HTML&warehouseId=MAD7&processId=1002960&spanType=Week&startDateWeek=${weekStart}&maxIntradayDays=1&startDateIntraday=${today}&startHourIntraday=15&startMinuteIntraday=0&endDateIntraday=${today}&endHourIntraday=23&endMinuteIntraday=0`;
    }
    ...
    // POD BANDS DASHBOARD
    // ─────────────────────────────────────────────────────────────────────────
    function createPodBandsDashboard() {
        const existingDashboard = document.getElementById('podbands-dashboard');
        if (existingDashboard) existingDashboard.remove();

        const dashboard = document.createElement('div');
        dashboard.id = 'podbands-dashboard';
        dashboard.className = 'fclm-dashboard-card';
        dashboard.style.cssText = `
            position: fixed;
            top: 304px;
        ...
        dashboard.onclick = function() {
            window.open(getPodBandsURL(), '_blank');
        };
        dashboard._setRefreshed = function() {
            lastRefreshed = new Date().toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' });
        };
        ...
        const refreshDot = document.createElement('div');
        refreshDot.className = 'fclm-refresh-dot';
        refreshDot.id = 'podbands-refresh-dot';
        dashboard.appendChild(refreshDot);

        const header = document.createElement('div');
        header.textContent = 'POD BANDS';
        header.style.cssText = `
            font-size: 9px;
            font-weight: bold;
            color: white;
        ...
        const podBandsCell = document.createElement('div');
        podBandsCell.style.cssText = `
            display: flex;
            flex-direction: column;
            align-items: center;
            justify-content: center;
        ...
        const podBandsValueDiv = document.createElement('div');
        podBandsValueDiv.id = 'podbands-value';
        podBandsValueDiv.style.cssText = `
            font-size: 16px;
            font-weight: 900;
            color: #ffffff;
            text-align: center;
            ...
            font-family: 'Courier New', monospace;
        `;
        podBandsValueDiv.innerHTML = '<span id="podbands-hours-number">---</span><span style="font-size:11px;font-weight:bold;color:#bdc3c7;">hrs.</span>';

        podBandsCell.appendChild(podBandsValueDiv);
        bodyRow.appendChild(podBandsCell);
        dashboard.appendChild(header);
        dashboard.appendChild(bodyRow);
        document.body.appendChild(dashboard);

        podBandsValueElement = document.getElementById('podbands-hours-number');

        console.log('[FCLM AA Time Checker] POD BANDS dashboard created at ' + new Date().toISOString());
    }
    ...
    // FETCH POD BANDS
    // ─────────────────────────────────────────────────────────────────────────
    function fetchPodBandsValue() {
        var url = getPodBandsURL();
        console.log('[FCLM AA Time Checker] Fetching POD BANDS from: ' + url);
        ...
        GM_xmlhttpRequest({
            ...
            onload: function(response) {
                if (response.status === 200) {
                    ...
                    // Find the Pod Banding table by its ID
                    var targetTable = doc.getElementById('function-1712591799567');

                    // Fallback: search by caption text if ID not found
                    if (!targetTable) {
                        doc.querySelectorAll('caption').forEach(function(caption) {
                            if (caption.textContent.includes('1712591799567') || caption.textContent.toLowerCase().includes('pod banding')) {
                                targetTable = caption.closest('table');
                            }
                        });
                    }
                    ...
                    if (targetTable) {
                        console.log('[FCLM AA Time Checker] Found Pod Banding table');
                        var podBandsHours = null;

                        // Look in tfoot for the "total empl-all" row
                        var tfoot = targetTable.querySelector('tfoot');
                        if (tfoot) {
                            var totalRow = tfoot.querySelector('tr.total.empl-all');
                            if (totalRow) {
                                // Get the td with class "size-total highlighted" and data-column="9"
                                var totalCell = totalRow.querySelector('td.size-total.highlighted[data-column="9"]');
                                if (totalCell) {
                                    var rawVal = totalCell.textContent.trim();
                                    var parsed = parseLocalizedNumber(rawVal);
                                    if (!isNaN(parsed)) {
                                        podBandsHours = parsed;
                                        console.log('[FCLM AA Time Checker] POD BANDS hours from tfoot: ' + podBandsHours);
                                    }
                                }
                            }
                        }

                        // Fallback: if tfoot approach didn't work, try any td.size-total.highlighted in a total row
                        if (podBandsHours === null) {
                            var allTotalRows = targetTable.querySelectorAll('tr.total.empl-all');
                            for (var r = 0; r < allTotalRows.length; r++) {
                                var cells = allTotalRows[r].querySelectorAll('td.size-total.highlighted');
                                for (var c = 0; c < cells.length; c++) {
                                    var rawVal2 = cells[c].textContent.trim();
                                    var parsed2 = parseLocalizedNumber(rawVal2);
                                    if (!isNaN(parsed2) && parsed2 > 0) {
                                        podBandsHours = parsed2;
                                        console.log('[FCLM AA Time Checker] POD BANDS hours (fallback): ' + podBandsHours);
                                        break;
                                    }
                                }
                                if (podBandsHours !== null) break;
                            }
                        }

                        if (podBandsHours !== null && podBandsValueElement) {
                            podBandsValueElement.textContent = podBandsHours.toFixed(2);
                            podBandsValueElement.style.color = '#ffffff';
                            var podBandsDash = document.getElementById('podbands-dashboard');
                            if (podBandsDash && podBandsDash._setRefreshed) podBandsDash._setRefreshed();
                            console.log('[FCLM AA Time Checker] POD BANDS updated to: ' + podBandsHours.toFixed(2));
                        } else {
                            console.log('[FCLM AA Time Checker] Could not extract POD BANDS value');
                        }
                    } else {
                        console.log('[FCLM AA Time Checker] Could not find Pod Banding table (ID: function-1712591799567)');
                    }
                } else {
                    console.log('[FCLM AA Time Checker] Failed to fetch POD BANDS (status: ' + response.status + ')');
                }
            },
            onerror: function() {
                console.log('[FCLM AA Time Checker] Error fetching POD BANDS data');
            }
        });
    }
    ...
    function init() {
        ...
        setInterval(function() { fetchPodBandsValue(); }, 600000);
